...
color ray_color(
  ...
  // === 特徴線チェック ===
    ...
    if (check_for_feature_line(r, rec, world, cam)) {
        // 論文の Observation 1: 特徴線を光源として扱う 
        // パスをここで終了させ、線の色を返す
        return FeatureLine::line_color; 
    }
...
int main() {
  ...
  for (int j = image_height-1; j >= 0; --j) {
    ...
    for (int i = 0; i < image_width; ++i) {
      ...
      // PNG用バッファに書き込み（画像の上下を反転）
      int buffer_index = (image_height - 1 - j) * image_width + i;
      write_color_to_buffer(image_buffer.data(), buffer_index, pixel_color, samples_per_pixel);
    }
  }

  // PNGファイルとして保存
  const char* filename = "output.png";
  if (stbi_write_png(filename, image_width, image_height, 3, image_buffer.data(), image_width * 3)) {
    std::cerr << "\nPNG file saved as: " << filename << "\n";
  } else {
